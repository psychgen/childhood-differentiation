...
prepareMplusData(mplusdat, "//tsd-evs/p471/data/durable/projects/childhood_differentiation/scripts/mplus/data/data_for_mlevel.dat")

# Run multilevel models (takes a long time! ~ 12 hours for all models )

filepath1 <- "./scripts/mplus/scripts/twolevel_lgm"
...
file.copy(from=paste0(filepath1,"/",list.files(filepath1)[str_detect(list.files(filepath1),".inp",negate=T)]),
          to="./scripts/mplus/output/twolevel_lgm",
...
mplusOutput <- readModels("./scripts/mplus/output/twolevel_lgm", recursive=FALSE,
...
# save 
save(diffests, file= "./output/diffests.RData")
load("./output/diffests.RData")

save(totests, file= "./output/totests.RData")
load("./output/totests.RData")
...
tiff("figures/all_preds_diff_adj.tiff", res = 800, compression = "lzw", unit = "in",
     height = 7, width = 10)
...
p1<-ggplot(totests2, mapping=aes(x=b_s_adjust_est,y=b_i_adjust_est,shape=parent,colour=category)) +
 ...
 tiff("figures/all_preds_tot_adj.tiff", res = 800, compression = "lzw", unit = "in",
     height = 7, width = 10)
