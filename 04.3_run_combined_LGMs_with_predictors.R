# 04.3_run_combined_LGMs_with_predictors.R
...
source("./scripts/04.2_specify_combined_LGMs_with_predictors.R")
...
standardizedSolution(fit_model1_both, type = "std.all", se = TRUE, 
...
ests1 <- ests1 %>%
  mutate(lhs=fct_relevel(lhs,"s1","i1"))
...
tmp3 <- standardizedSolution(fit_model1_both, ci = TRUE, level = 0.95) %>%
...
# fix order of slope and intercept
ests3 <- ests3 %>%
  mutate(lhs=fct_relevel(lhs,"s2","i2"))
